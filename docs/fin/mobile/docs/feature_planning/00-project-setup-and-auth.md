...
- `src/services/api.ts` — Axios client: base URL (`__DEV__` → `http://localhost:3002`, prod → `https://fin-api.derekentringer.com`), `X-Client-Type: mobile` header on all requests, expo-secure-store token management (keys: `fin_access_token`, `fin_refresh_token`, `fin_token_expiry`), request interceptor for Bearer auth with proactive refresh (30s buffer), response interceptor for 401 auto-refresh, shared refresh lock to prevent concurrent refresh calls, 10 auth API methods (getMe, verifyTotp, setupTotp, verifyTotpSetup, disableTotp, changePassword, revokeAllSessions, register, forgotPassword, resetPassword)
- `src/store/authStore.ts` — Zustand: isAuthenticated, isLoading, user; actions: initialize() (restore from secure store, attempt refresh if expired, fetch user via getMe), login() (returns TOTP challenge state), logout(), verifyTotp(), setUser(); selector hooks: useIsAuthenticated(), useIsLoading(), useUser()
