...
Single-user authentication for the web app, reusing the `@derekentringer/shared` auth plugin. Same JWT + bcrypt pattern as fin-web.
...
- **Auth flow**:
  ...
  - Refresh token stored as HttpOnly cookie (same as fin-web)
...
- **Token management**:
  ...
  - Same interceptor pattern as fin-web `client.ts`
- **Route protection**:
  - All routes except `/login` require authentication
  - `AuthContext` provider wraps the app (same pattern as fin-web)
...
- Auth routes in ns-api: copy structure from fin-api `src/routes/auth.ts` (same endpoints, same logic)
- No PIN layer — simpler than fin-api (PIN is finance-specific for sensitive financial operations)
