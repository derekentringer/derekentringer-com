...
### CodeBlock Language Detection (`ns-web`)
...
| File | Change |
|------|--------|
| `packages/ns-web/package.json` | Added `mermaid` dependency |
| `packages/ns-web/src/components/MermaidDiagram.tsx` | **New** — Lazy-loaded mermaid renderer with theme support |
| `packages/ns-web/src/components/CodeBlock.tsx` | Added language detection, delegate mermaid to MermaidDiagram |
| `packages/ns-web/src/components/QAPanel.tsx` | Added `components={{ pre: CodeBlock }}` for mermaid + copy button |
| `packages/ns-web/src/styles/global.css` | Added `.mermaid-diagram` / `.mermaid-error` / `.mermaid-loading` CSS |
