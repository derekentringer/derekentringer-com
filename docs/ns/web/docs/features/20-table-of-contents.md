# 20 — Table of Contents Panel

## Summary

Notes with headings now have a Table of Contents tab in the right-side drawer (alongside AI Assistant and Version History). The TOC shows document structure extracted from markdown headings (h1–h6) and allows click-to-scroll navigation to any heading in the preview pane. Uses `rehype-slug` for heading IDs in rendered HTML and `github-slugger` for matching slug generation in the TOC panel. Applies to both ns-web and ns-desktop.

## What Was Built

### extractHeadings Utility (`ns-web`)
...
### MarkdownPreview rehype-slug Integration (`ns-web`)
...
### NotesPage Drawer Integration (`ns-web`)
- **`DrawerTab`** type expanded to `"assistant" | "history" | "toc"`
- TOC tab button with Lucide "list" icon (lines with dots), visible when `selectedId && sidebarView === "notes"`
...
| File | Change |
|------|--------|
| `packages/ns-web/package.json` | Added `rehype-slug` + `github-slugger` dependencies |
| `packages/ns-web/src/lib/extractHeadings.ts` | **New** — Heading extraction with slug generation |
| `packages/ns-web/src/components/TocPanel.tsx` | **New** — TOC drawer panel component |
| `packages/ns-web/src/components/MarkdownPreview.tsx` | Added `rehypeSlug` to rehype plugins |
| `packages/ns-web/src/pages/NotesPage.tsx` | Expanded DrawerTab, added TOC tab + panel + scroll handler |
