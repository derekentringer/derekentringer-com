...
- [x] Per-tab cursor and scroll position caching — Switching between editor tabs preserves and restores both cursor position and scroll offset; `pendingCursorRef` + `pendingScrollRef` in MarkdownEditor consumed atomically in the value-sync `useEffect` dispatch (same transaction as content replacement, with `Transaction.addToHistory.of(false)` to prevent undo history leaking between notes); scroll restored synchronously after dispatch (not via `requestMeasure` which races with user mouse interactions causing selection highlighting); separate `tabEditorStateRef` in NotesPage stores `{ cursor, scrollTop }` per tab, cleaned up on tab close; cursor clamped to doc length when sync changes content of a background tab; pending refs always cleared even when content hasn't changed to prevent stale consumption
