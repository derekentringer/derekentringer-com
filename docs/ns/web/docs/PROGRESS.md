...
- [x] [25 — Live Meeting Assistant](features/25-meeting-assistant.md) — Real-time AI meeting assistant: 20-second chunked Whisper transcription during recording, pgvector semantic note matching every 45 seconds via Voyage embeddings, integrated into AI Assistant panel (renamed from QAPanel) with collapsible "Related Notes" + resizable "Transcription" sections with typing animation, mode-aware status text ("Monitoring the [mode] to surface related notes...") with bouncing dots animation, "Catch me up" button for instant meeting summary, live Q&A with transcript-aware context (`answerMeetingQuestion`), dynamic recording-ended card (mode-aware header, clickable note title, key topic pills from note headings, bouncing dots during processing), compact chat-markdown styling (13px, tighter spacing), raw transcript saved to `transcript` database column (Postgres + SQLite) and viewable via mic icon button on note toolbar opening read-only TranscriptViewer, desktop native audio chunk export from Rust (system+mic mixed WAV), FolderPicker component extracted for recording bar folder selection (stripped dot prefix from selected display), dashboard skeleton loading placeholders, `structureTranscript` max_tokens increased to 8192 for long meetings, desktop wiki-link fix (server PATCH instead of local updateNote to avoid FTS constraint)
...
- [x] Agentic tool use — Claude tool_use API with 22 tools (search, list folders/tags, stats, recent, content, backlinks, open, create, update content, move, tag, generate tags/summary, delete note/folder, toggle favorite, list trash, restore note, rename folder, rename tag, duplicate note), tool activity indicators with bouncing dots, interactive note cards in chat responses (accumulated + re-sent for persistence), SSE sync notify after write operations, system prompt with slash command awareness, max 3 tool rounds per question, tools available during meetings (transcript injected into system prompt)
- [x] Slash commands — 21 client-side commands (`/open`, `/create`, `/move`, `/tag`, `/delete`, `/deletefolder`, `/summarize`, `/gentags`, `/favorites`, `/favorite`, `/unfavorite`, `/trash`, `/restore`, `/renamefolder`, `/renametag`, `/duplicate`, `/recent`, `/folders`, `/tags`, `/stats`, `/clear`), autocomplete dropdown with arrow keys/Tab/Enter/Escape, slash commands in messages rendered with code blocks
