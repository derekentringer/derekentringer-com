...
- `start_recording(app_handle)` — sequential setup: (1) mic permission pre-request, (2) 500ms delay, (3) system audio permission pre-request via minimal global tap, (4) 500ms delay, (5) create `mpsc::sync_channel(128)` pairs for system + mic, (6) spawn writer threads streaming to `/tmp/notesync_sys_{ts}.pcm` and `/tmp/notesync_mic_{ts}.pcm`, (7) mic AudioUnit via `default_input_device`, (8) system audio process tap (no dialog — permission already granted), (9) aggregate device from tap, (10) system AudioUnit on aggregate device, (11) timer thread emitting `"meeting-recording-tick"` events
...
- `mix_to_wav(sys_path, mic_path, ...)` — reads both raw PCM temp files in 1-second chunks, converts each chunk to mono (`to_mono`), downsamples both streams to 16kHz via `ChunkResampler` (Whisper's native rate — no quality loss), mixes by averaging, writes WAV via `hound::WavWriter` (16-bit signed int, 16kHz, mono); ~2MB working set regardless of recording length, ~1.9 MB/min output file size
