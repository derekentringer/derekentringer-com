...
- [x] Per-tab cursor and scroll position caching — Switching between editor tabs preserves and restores both cursor position and scroll offset; `pendingCursorRef` + `pendingScrollRef` in MarkdownEditor consumed atomically in the value-sync `useEffect` dispatch (same transaction as content replacement, with `Transaction.addToHistory.of(false)` to prevent undo history leaking between notes); scroll restored via `requestMeasure` after CM finishes measuring new content (CM6 virtualizes rendering so synchronous `scrollTop` gets clamped to old document height); separate `tabEditorStateRef` in NotesPage stores `{ cursor, scrollTop }` per tab, cleaned up on tab close; cursor clamped to doc length when sync changes content of a background tab; pending refs always cleared even when content hasn't changed to prevent stale consumption; `preventFocusScroll` ViewPlugin captures `scrollTop` on focus and restores it after CM's update cycle to prevent WKWebView focus-triggered scroll-to-cursor from jumping the viewport when clicking into the editor after scrolling
