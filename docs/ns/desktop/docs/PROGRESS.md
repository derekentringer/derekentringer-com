...
- [x] [10d — AI Features: Audio Notes](features/10d-ai-audio-notes.md) — Audio recording via MediaRecorder API, transcription via ns-api Whisper endpoint, four modes (Meeting/Lecture/Memo/Verbatim), AudioRecorder component with timer + mode dropdown, note timestamps in editor toolbar status bar; meeting audio recording via Core Audio Taps (macOS 14.2+) using `cpal` crate loopback capture for system audio + standard input for microphone, two concurrent streams mixed at stop time, `RecordingSource` setting (`microphone`/`meeting`), Rust `audio_capture.rs` module with `cpal` + `hound` crates, three Tauri commands, WAV output at native sample rate mono, recording source selector in dropdown + Settings page
...
- [x] Core Audio Tap migration — Migrated meeting audio recording from ScreenCaptureKit (`screencapturekit` crate) to Core Audio Taps (`cpal` crate v0.17). ScreenCaptureKit broke on macOS Tahoe (26.x) due to stricter TCC enforcement for "Screen & System Audio Recording" — `SCShareableContent::get()` fails even with permission granted (requires app restart, weekly re-confirmation). Core Audio Taps fall under the less-intrusive "System Audio Recording Only" TCC category. Changes: replaced `screencapturekit` with `cpal` in Cargo.toml, removed Swift runtime rpath from build.rs, rewrote `audio_capture.rs` to use two concurrent cpal streams (loopback + mic) with mono conversion + resampling + mixing, replaced `NSScreenCaptureUsageDescription` with `NSAudioCaptureUsageDescription` in Info.plist, lowered macOS version requirement from 15.0+ to 14.2+. Added permission pre-request functions: `request_microphone_permission()` via AVCaptureDevice (`objc2` + `block2`) and `request_system_audio_permission()` via `AudioHardwareCreateProcessTap` (`objc2-core-audio`) — called before any cpal operations to ensure exactly one macOS permission dialog per permission type instead of multiple stacked dialogs. Zero frontend changes — same Tauri commands, events, and return types.
