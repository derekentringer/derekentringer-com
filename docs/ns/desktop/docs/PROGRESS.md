...
- [x] Image support via Cloudflare R2 — Same as web (paste/drop upload, R2 storage, AI vision descriptions, sync protocol, resizable images with drag handles, lightbox, context menu, image grid), plus: offline-first image upload queue saves images to local `image-cache/` directory with `pending_upload` sync status and `notesync-local://` placeholder URLs; on reconnect, sync engine reads cached binary, uploads via API, updates local DB with real R2 URL, and replaces placeholder URLs in note content; SQLite migration 011 adds `images` table; `upsertImageFromRemote`/`softDeleteImageFromRemote` in db.ts for sync pull; `applyImageChange` in sync engine handles incoming image changes
...
- [x] Trash sync fix — Upsert note/folder/image before soft-deleting during sync pull so items trashed before desktop synced them now appear in desktop trash
...
- [ ] [26b — Frontmatter Properties Panel](feature_planning/26b-frontmatter-properties-panel.md) — Obsidian-style Properties panel above the editor with structured form inputs for frontmatter fields; raw YAML source toggle; CodeMirror fold extension to hide frontmatter in panel mode; bidirectional sync between panel and content
