...
7. **macOS: system audio silently silent under `tauri dev`.** `tauri dev` runs `target/debug/NoteSync` with an unstable ad-hoc code signature (the signature changes on every Rust rebuild). macOS TCC keys **System Audio Recording** permission by bundle path + code signature together, so every dev rebuild looks like a "different app" — the Settings UI still shows the bundle ID enabled, but the aggregate-device + process-tap audio unit callbacks receive silent samples. Symptoms: meeting-mode recording produces 10+ MB of PCM that sounds silent, Whisper hallucinates YouTube subscribe-outro text (often in Chinese or Japanese), top-ribbon waveform animates for mic-only. **Fix**: for any audio-related testing, use a packaged build (`npm run tauri:build` → `src-tauri/target/release/bundle/macos/NoteSync.app`). The `.app`'s signature is stable across rebuilds of the same version, so TCC grants stick. `tauri dev` is still fine for UI/frontend iteration — just not for meeting-mode audio verification.
