...
Background sync engine for mobile, keeping the local SQLite database in sync with central PostgreSQL via ns-api. Same protocol as the desktop sync engine.
...
- Consider extracting sync protocol into `ns-shared` package with platform-specific SQLite adapters
