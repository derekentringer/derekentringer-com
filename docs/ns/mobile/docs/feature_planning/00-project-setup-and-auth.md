...
- **Project setup** (`packages/ns-mobile/`):
...
- **API client** (`src/services/api.ts`):
  - Base URL: `__DEV__ ? "http://localhost:3004" : "https://ns-api.derekentringer.com"`
...
- Copy patterns from existing `packages/fin-mobile/` (fin mobile): API service, auth store, token storage, interceptors
