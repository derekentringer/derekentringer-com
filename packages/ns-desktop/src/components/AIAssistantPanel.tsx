...
interface MeetingSummaryData {
   ...
   *  the right card when a session result arrives — critical once multiple
   *  processing tasks can be in flight concurrently (Phase 1 detach). */
  sessionId?: string;
  /** Phase 2: explicit lifecycle so the card can render a failed state with
   *  Retry/Discard. Pre-Phase-2 persisted cards omit this; code must tolerate
   *  missing status (treat as `processing` when noteId absent, else completed). */
...
interface AIAssistantPanelProps {
  ...
  /** Phase 2: unified success/failure result. The panel matches by
   *  sessionId and either enriches the card (success) or flips it to a
   *  failed state with Retry/Discard affordances (failure). */
   ...
   *  summary card when recording stops so results can match by id. */
  ...
  /** Re-run processing for a previously-failed session. The parent routes
   *  this into AudioRecorder's snapshot-backed retry. */
   ...
   *  cards and prior-session cards after a window reload — even
   ...
   *  its stale in-memory copy. (Sidebar list refresh still happens
   *  via the sync-engine `onDataChanged` path.) */
...
export function AIAssistantPanel({ onSelectNote, isOpen, isRecording, isSearchingContext, liveTranscript, relevantNotes, recordingMode, audioSessionResult, activeNote, chatRefreshKey, activeSessionId, cancelledSessionId, onAudioRetry, onAudioDiscard, canAudioRetry, autoApprove, focusNonce, onNoteContentRewritten }: AIAssistantPanelProps) {
  ...
  // card loaded from server history is stale — the run that would have
  // enriched it is gone. Repaint as "failed" with a helpful message. Only
  // applies to Phase 2+ cards that carry an explicit `status` field; older
  // cards without `status` keep their current shape (noteId absent ≈ still
  // working on upgrade, but there's no snapshot to retry from).
  ...
  // jobs, a "processing" card persisted in chat history represents a
  // server-side job still in flight; the SSE `transcription-job` event
  // delivers the terminal status (and the real server error message)
  ...
  function rowToMessage(r: ChatMessageData): Message {
    // Mobile-originated cards may omit `relevantNotes` / `transcript`
    // when nothing populated them; the renderer below dereferences
    // both directly (`.length`, `.trim()`). Coerce missing fields
    // to empty values here so the card stays render-safe.
    let meetingData =
  ...
  // cards can coexist and the wrong one would be updated otherwise.
  useEffect(() => {
    ...
    setMessages((prev) => {
      ...
      // Fallback: pre-Phase-1 persisted cards lack sessionId. Only used for
      // success results since failures always carry a sessionId from Phase 2.
  ...
  // when the user clicks Retry. The actual processSession run is invoked by
  // the parent via onAudioRetry; when it settles, the audioSessionResult
  // effect above will update the card again.
  ...
  const commandCtx = useMemo((): CommandContext => ({
    ...
    renameNote: async (oldTitle, newTitle) => {
      try {
        const notes = await searchNotes(oldTitle);
        const note = notes.find((n) => n.title === oldTitle) ?? notes.find((n) => n.title.toLowerCase() === oldTitle.toLowerCase());
  ...
  async function handleConfirmApply(idx: number, pendings: PendingConfirmation[]) {
    ...
    for (const pending of pendings) {
      try {
        ...
        // the open editor can reload. Sidebar list refresh is handled
        // separately via the sync-engine onDataChanged callback.
        // The confirm response's noteCards carries the authoritative
        // note id — prefer it over the preview title, which could
        // collide if two notes share a title.
        if (
    ...
    <div className="flex flex-col h-full bg-background" data-testid="qa-panel">
      ...
      <div className="px-3 py-2 border-b border-border shrink-0 flex items-center gap-1.5">
        ...
        {/* Phase E.1 was a header indicator; moved inline at the
            bottom of the conversation (see ThinkingBubble below) so
            it sits in the user's natural reading position. */}
        {isSearchingContext && (
          ...
          >
            {msg.confirmation ? (
              ...
              <div className="w-full rounded-lg bg-card border border-border p-3 animate-fade-in">
                ...
                {/* Status-driven body: completed → clickable note card;
                 *  failed → error + Retry/Discard; processing → spinner. */}
