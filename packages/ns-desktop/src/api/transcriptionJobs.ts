// Phase H — Server-managed transcription jobs (desktop client).
//
// Desktop uploads audio (or a prebuilt live transcript for the
// fast path) once to /ai/transcribe-jobs and the server owns
// the rest of the pipeline. Desktop uses `XMLHttpRequest` for
// the audio upload because XHR exposes `upload.onprogress`
// events — Tauri's webview-based runtime supports XHR the same
// way browsers do, so the implementation matches ns-web verbatim.
//
