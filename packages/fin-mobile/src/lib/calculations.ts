...
export const IRS_401K_LIMIT = 23500;
...
export function calculateFourOhOneK(
  ...
  // Current level
  const currentAnnualContribution = Math.min(
    (annualSalary * currentContributionPct) / 100,
    IRS_401K_LIMIT,
  );
  const currentEmployerMatch = computeEmployerMatch(
  ...
  const optimalAnnualContribution = Math.min(
    (annualSalary * optimalContributionPct) / 100,
    IRS_401K_LIMIT,
  );
  const optimalEmployerMatch = computeEmployerMatch(
  ...
  const maxEmployerMatch = computeEmployerMatch(
  ...
  for (let y = 1; y <= 30; y++) {
    currentBal =
      (currentBal + currentAnnualContribution + currentEmployerMatch) *
      (1 + returnRate);
    optimalBal =
      (optimalBal + optimalAnnualContribution + optimalEmployerMatch) *
      (1 + returnRate);
    maxBal =
      (maxBal + maxAnnualContribution + maxEmployerMatch) * (1 + returnRate);
