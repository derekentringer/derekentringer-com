...
describe("createMobileTokenAdapter", () => {
  ...
  describe("doRefresh", () => {
    it("returns null when no refresh token in storage", async () => {
      ...
      const result = await adapter.doRefresh("http://localhost:3002");
    ...
    it("sends refresh token in request body and returns result on success", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("http://localhost:3002/auth/refresh", {
        method: "POST",
        headers: {
    ...
    it("throws on transient server error (preserves session)", async () => {
      ...
      await expect(adapter.doRefresh("http://localhost:3002")).rejects.toThrow(
