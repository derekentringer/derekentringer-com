...
describe("tokenStorage", () => {
  ...
  describe("getAccessToken", () => {
    it("reads from SecureStore", async () => {
      ...
      expect(mockGetItemAsync).toHaveBeenCalledWith("fin_access_token");
    });
  });
  ...
  describe("getRefreshToken", () => {
    it("reads from SecureStore", async () => {
      ...
      expect(mockGetItemAsync).toHaveBeenCalledWith("fin_refresh_token");
    });
  });
  ...
  describe("setTokens", () => {
    it("stores tokens in SecureStore and sets in-memory token", async () => {
      ...
      expect(mockSetItemAsync).toHaveBeenCalledWith("fin_access_token", "access-123");
      expect(mockSetItemAsync).toHaveBeenCalledWith("fin_refresh_token", "refresh-456");
  ...
  describe("clearAll", () => {
    it("clears SecureStore and in-memory token", async () => {
      ...
      expect(mockDeleteItemAsync).toHaveBeenCalledWith("fin_access_token");
      expect(mockDeleteItemAsync).toHaveBeenCalledWith("fin_refresh_token");
