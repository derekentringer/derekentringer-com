...
npx turbo run dev --filter=@derekentringer/fin-api

# Start the app on a connected Android device (from packages/fin-mobile/)
...
```bash
# From packages/fin-mobile/
npm run type-check

# Or from repo root
npx turbo run type-check --filter=@derekentringer/fin-mobile
```
