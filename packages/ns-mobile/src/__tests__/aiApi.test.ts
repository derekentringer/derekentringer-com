...
describe("ai api client (mobile)", () => {
  ...
  describe("transcribeChunk", () => {
    it("POSTs FormData with sessionId/chunkIndex (no explicit timeout)", async () => {
      mockPost.mockResolvedValue({
      ...
      // No explicit Content-Type AND no timeout — axios's default
      // 0 waits indefinitely so long recordings aren't capped by
      // a client-side ceiling.
  ...
  describe("transcribeAudio", () => {
    it("POSTs FormData with mode + optional folderId (no explicit timeout)", async () => {
      mockPost.mockResolvedValue({
