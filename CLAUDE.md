...
- **Windows Rust cross-compile gotcha**: `RunEvent::Opened` (file-association open events) is macOS-only and must be gated behind `#[cfg(target_os = "macos")]` in `src-tauri/src/lib.rs` — otherwise Windows builds fail with `E0599: no variant named 'Opened'`. `audio_capture.rs` (CoreAudio) is already properly gated.
