...
- **Time range**: User-selectable (1M, 3M, 6M, 12M, YTD, All) with per-chart pill-style controls; each chart manages its own range independently; all dashboard charts default to 12M (Net Worth, Income vs Spending, favorite account balance charts)
